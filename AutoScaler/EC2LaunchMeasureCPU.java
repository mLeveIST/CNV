...
cloudWatch = AmazonCloudWatchClientBuilder.standard().withRegion("us-east-1").withCredentials(new AWSStaticCredentialsProvider(credentials)).build();
    ...
    runInstancesRequest.withImageId("ami-04ce232ab304186d9")
    ...
    public static void main(String[] args) throws Exception {
        EC2LaunchMeasureCPU ec2Launch = new EC2LaunchMeasureCPU();
        List<Instance> instancesToTerminate = new ArrayList<Instance>();
        ...
        try {
            ...
            double totalAverageCPU = 0.31;
            List<Instance> instancesLowCPU = new ArrayList<Instance>();
            double cpuUsage = 0.0;
            int count;
            ...
            for (Instance instance : instances) {
                ...
                if (state.equals("running")) {
                    ...
                    .withEndTime(new Date());
                     ...
                     if (count != 0) {
                        instanceAverageCPU = instanceAverageCPU/count;
                        cpuUsage += instanceAverageCPU;
                        // && instancesToTerminate.contains(instance)
                        if(instanceAverageCPU < 0.10  && instances.size() > 1){
                          System.out.println("Terminating Instance");
                          ec2Launch.terminateInstance(instance);
                          instancesToTerminate.remove(instance);
                        }
                        else if(instanceAverageCPU < 0.20){
                          instancesLowCPU.add(instance);
                        }
            ...
            if(instances.size() != 0){totalAverageCPU = cpuUsage/instances.size();}

            if( totalAverageCPU < 30.0) {

              for(Instance lowInstance : instancesLowCPU) {
                System.out.println("Flagging to Terminate!");
                //TODO, REMOVE FROM LB MAP
                instancesToTerminate.add(lowInstance);
              }

            }
            else if( totalAverageCPU > 0.80) {
